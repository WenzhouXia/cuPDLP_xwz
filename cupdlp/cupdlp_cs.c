                                    ...
                                    int triplet)
{
  ...
  A->nzmax = nzmax = MAX(nzmax, 1);
  A->nz = triplet ? 0 : 1; /* allocate triplet or comp.col */
  A->p = cupdlp_dcs_malloc(triplet ? nzmax : m + 1, sizeof(int));
  cupdlp_printf("Ap_len: %d\n", m + 1);
  A->i = cupdlp_dcs_malloc(nzmax, sizeof(int));
  A->x = values ? cupdlp_dcs_malloc(nzmax, sizeof(double)) : NULL;
  return ((!A->p || !A->i || (values && !A->x)) ? cupdlp_dcs_spfree(A) : A);
}
...
int cupdlp_dcs_sprealloc(cupdlp_dcs *A, int nzmax)
{
  ...
  if (!A)
    return (0);
  if (nzmax <= 0)
    nzmax = IS_CSC(A) ? (A->p[A->n]) : A->nz;
  nzmax = MAX(nzmax, 1);
  A->i = cupdlp_dcs_realloc(A->i, nzmax, sizeof(int), &oki);
  if (IS_TRIPLET(A))
    A->p = cupdlp_dcs_realloc(A->p, nzmax, sizeof(int), &okj);
  if (A->x)
    A->x = cupdlp_dcs_realloc(A->x, nzmax, sizeof(double), &okx);
  ok = (oki && okj && okx);
  if (ok)
    A->nzmax = nzmax;
  return (ok);
...
{
  if (!IS_TRIPLET(T) || i < 0 || j < 0)
    return (0); /* check inputs */
  if (T->nz >= T->nzmax && !cupdlp_dcs_sprealloc(T, 2 * (T->nzmax)))
    return (0);
  if (T->x)
    T->x[T->nz] = x;
  T->i[T->nz] = i;
...
cupdlp_dcs *cupdlp_dcs_compress(const cupdlp_dcs *T)
{
  ...
  if (!C || !w)
    return (cupdlp_dcs_done(C, w, NULL, 0)); /* out of memory */
  Cp = C->p;
  Ci = C->i;
  Cx = C->x;
  for (k = 0; k < nz; k++)
...
cupdlp_dcs *cupdlp_dcs_csr_transpose(const cupdlp_dcs *A, int values)
{
  ...
  Ai = A->i;
  Ax = A->x;
  C = cupdlp_dcs_spalloc(n, m, Ap[m], values && Ax, 0); /* allocate result */
  w = cupdlp_calloc(n, sizeof(cupdlp_int));             /* get workspace */
  if (!C || !w)
    return (cupdlp_dcs_done(C, w, NULL, 0)); /* out of memory */
  Cp = C->p;
  Ci = C->i;
  ...
  for (p = 0; p < Ap[m]; p++)
    w[Ai[p]]++;                    /* row counts */
  cupdlp_dcs_csr_cumsum(Cp, w, n); /* row pointers */
  for (j = 0; j < m; j++)
  {
    for (p = Ap[j]; p < Ap[j + 1]; p++)
    {
